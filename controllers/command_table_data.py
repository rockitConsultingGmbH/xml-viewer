...
from database.utils import insert_into_location, select_from_command, update_location, select_from_location
...
class LocationTableData:
    ...
    # Target Location
    def populate_command_fields(self, communication_id, parent_widget=None):
        if parent_widget is None:
            parent_widget = self.parent_widget
        ...
        conn = self.conn_manager.get_db_connection()
        cursor = conn.cursor()
        command_rows = select_from_command(cursor, communication_id).fetchall()
        for command_row in command_rows:
            if command_row:
                pass
                self.populate_command_fields(command_row)
        conn.close()
        logging.info("Target fields populated for communication_id: %s", communication_id)

    def populate_command_fields(self, command_row):
        logging.info("Populating target fields with data: %s", command_row)
        (id, communication_id, location, location_id, use_local_filename,
         use_path_from_config, target_must_be_archived, target_history_days,
         rename_existing_file, userid, password, description, location_type) = command_row
    ...
    def save_command_data(self, communication_id):
        logging.info("Saving target location data for communication_id: %s, location_type: %s", communication_id)
        conn = self.conn_manager.get_db_connection()
        cursor = conn.cursor()

        # Fetch existing locations from the database for the communication_id
        cursor = select_from_command(cursor, communication_id)
    ...
    def create_command_row(self, location_data, communication_id, location_type, is_new=False):
